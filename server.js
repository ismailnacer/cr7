...
client.on("ready", () => {
  console.log(`----------------`);
  console.log(`ismail nacer`);
  console.log(`----------------`);
  console.log(
    `ON ${client.guilds.size} Servers '     Script By : ismail nacer `
  );
  console.log(`----------------`);
  console.log(`Logged in as ${client.user.tag}!`);
  client.user.setActivity("=help II الكريدت تابع  للبرو بوت")
...
**  =giv •   يسويلك قف اوي علي الشي الي تبيه**
  ...
  if (args[0].toLowerCase() == `${prefix}credits`) {
    ...
    if (!args[2]) {
      ...
      const background = await Canvas.loadImage(
        "https://cdn.discordapp.com/attachments/749258555563180114/749363132304195604/cr.PNG"
      );
      ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
      ctx.font = "25px Arial Bold";
      ctx.fontSize = "10px";
      ctx.fillStyle = "#0688F4";
      message.channel
        .send(
          `**${
            message.author.username
...
type these numbers to confirm: `
  ...
  if (args[0].toLowerCase() === `${prefix}daily`) {
    ...
    if (Daily !== null && cooldown - (Date.now() - Daily) > 0) {
      ...
      let ammount = (753, 522, 239, 800, 277, 457, 555, 639, 799, 587);
